...
if __name__ == "__main__":
    ...
    dT_dx = torch.autograd.grad(outputs=T, inputs=x, grad_outputs=torch.ones_like(T), create_graph=True)[0]
    dT_dt = torch.autograd.grad(outputs=T, inputs=t, grad_outputs=torch.ones_like(T), create_graph=True)[0]
    print("一阶导:\n", dT_dx, dT_dt)
    d2T_dx2 = torch.autograd.grad(outputs=dT_dx, inputs=x, grad_outputs=torch.ones_like(dT_dx), create_graph=True)[0]
    print("二阶导:\n", d2T_dx2)