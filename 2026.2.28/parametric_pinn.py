...
alpha_eval = torch.full_like(X_flat, alpha_test)       # 固定 α 值, shape 与 X_flat 一致
...
X_np = X.numpy()
T_np = T_grid.numpy()
alpha_val = alpha_test  # 验证时必须与网络预测用的 α 一致
...
plt.tight_layout()
plt.savefig('parametric_pinn_result.png', dpi=150, bbox_inches='tight')
print("图片已保存为 parametric_pinn_result.png")